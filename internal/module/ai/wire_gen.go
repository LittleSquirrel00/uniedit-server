...
func InitializeModule(config *Config) (*Module, error) {
	...
	manager := group.NewManager(groupRepository)
	routingManager := ProvideRoutingManager(registry, healthMonitor, manager)
	taskManager := ProvideTaskManager(taskRepository, config)
	embeddingCache := ProvideEmbeddingCache(config)
	service := llm.NewService(registry, healthMonitor, routingManager)
	mediaService := ProvideMediaService(registry, healthMonitor, taskManager)
...
var CoreSet = wire.NewSet(adapter.GetRegistry, ProvideProviderRegistry,
	ProvideHealthMonitor, group.NewManager, ProvideRoutingManager,
	ProvideTaskManager,
)

// ServiceSet contains service providers.
var ServiceSet = wire.NewSet(llm.NewService, ProvideMediaService)

// HandlerSet contains handler providers.
var HandlerSet = wire.NewSet(handler.NewChatHandler, handler.NewMediaHandler, handler.NewTaskHandler, handler.NewAdminHandler, ProvideHandlers)
